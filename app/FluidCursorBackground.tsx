...
export default function FluidCursorBackground() {
  ...
  useEffect(() => {
    ...
    // Small delay ensures CSS has been painted and the browser window is painted
    ...
    let timeoutId = setTimeout(() => {
      ...
      `);
          ...
          function render(target) {
            ...
            if (target == null && config.TRANSPARENT) {
              // Removed checkered debug transparency background
            }
            if (config.SHADING) {
          ...
          function resizeCanvas() {
            if (canvas.width != (canvas.clientWidth || window.innerWidth) || canvas.height != (canvas.clientHeight || window.innerHeight)) {
              canvas.width = (canvas.clientWidth || window.innerWidth);
              canvas.height = (canvas.clientHeight || window.innerHeight);
              initFramebuffers();
            }
          ...
          on(window, 'mousemove', e => {
            ...
            if (e.target && e.target.closest && (e.target.closest('.site-header') || e.target.closest('nav'))) {
              p.x = x; p.y = y;
              p.moved = false;
              return;
            }
          ...
          on(window, 'touchmove', e => {
            if (e.target && e.target.closest && (e.target.closest('.site-header') || e.target.closest('nav'))) return;
        ...
        destroyFunc = fluidSimulation(canvas);
      } catch (err) {
        console.warn("Fluid WebGL animation failed to initialize:", err);
      }
    }, 80);
  ...
  return <canvas ref={canvasRef} suppressHydrationWarning className="site-fluid-bg" aria-hidden="true" style={{ position: 'fixed', inset: 0, width: '100vw', height: '100vh', zIndex: 9999, pointerEvents: 'none' }} />;
}
