...
test("server-renders the OneBonsai Gulf experience", async () => {
  ...
  assert.match(html, /One secure source of context/);
...
test("keeps high-resolution scroll media, UAE imagery, and customer identities in source", async () => {
  ...
  assert.match(page, /editorial-connect-systems-artlist-v1\.mp4/);
  assert.match(page, /editorial-build-useful-ai-artlist-v1\.mp4/);
  assert.match(page, /editorial-scale-with-control-artlist-v1\.mp4/);
  ...
  assert.match(integrationMap, /One secure source of context/);
  ...
  assert.match(css, /\.integration-scroll-visual > video/);
